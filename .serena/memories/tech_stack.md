...
- `sim-cluster` IPC (provisional): `shared_memory` 0.12 + `raw_sync` 0.1 for SHM Event wake — unmaintained; swap planned (`TODO(ipc)` in `sim-cluster/src/shm_uart.rs`). Also `crossbeam-queue`, `libc` (flock).
