...
- `gdb`: QEMU-style GDB stub via the `gdbstub` crate (`listen_gdb`, `parse_gdb_dev`). `SimGdbTarget` implements `Target` / resume / SW breakpoints over `Command`/`Response`. `NotifyHalt` is a no-op placeholder for a future multi-board arbiter. `tcp::PORT` binds `0.0.0.0`.
...
- `command`: `Command` / `Response` / inspect surface for host front-ends (`Step`, `NotifyHalt` included).
