...
- `wiring`: `Wire<T, C, Sinks, Sources>` (typenum; 1-N or N-1) / `SourcePort<T, C>`. Ports hold the wire `Arc`. `drive(value, &mut C)` calls `WireSink<T, C>::on_input` immediately with the sink component. Combinational chaining is allowed. Loop detection is not implemented.
